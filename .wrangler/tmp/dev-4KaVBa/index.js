...
// .wrangler/tmp/bundle-z2BM1C/checked-fetch.js
...
var Factory = class {
  ...
  createApp = /* @__PURE__ */ __name((options) => {
...
var Optional = class _Optional {
  ...
  map(mapper) {
    ...
    return _Optional.of(null);
  }
  // Equivalent to .orElseThrow()
  orElseThrow(message = "Resource not found", status = 400) {
    if (!this.isPresent()) {
...
userRoutes.get("/:id", ...factory.createHandlers(async (c) => {
  const id = Optional.of(c.req.param("id")).orElseThrow("ID is required", 400);
  console.log(`Searching for user with ID: ${id}`);
  const user = await userService.getUserById(id);
  const safeUser = Optional.of(user).orElseThrow("User not found", 404);
...
app.get("/test", (c) => c.text("Hono is working on Workers!"));
var src_default = app;
...
// .wrangler/tmp/bundle-z2BM1C/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-z2BM1C/middleware-loader.entry.ts
